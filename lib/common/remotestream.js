...
function RemoteStream(socket, id, wait, options) {
  this._options = options || {
    readableObjectMode: false,
    writableObjectMode: false
  };
  RemoteStream.super_.call(this, options);
...
RemoteStream.prototype._write = function (chunk, encoding, callback) {
  ...
  // In objectmode we do not really have a buffer, just pass in the string
  if (client && chunk.buffer) {
    chunk = chunk.buffer;
  }

  function doWrite() {
    if (self._options.writableObjectMode || self._options.objectMode) {
      debug('%s writing object', self.id, chunk);
    } else {
...
RemoteStream.prototype._onWrite = function (chunk) {
  if (this._readableState.ended) {
    ...
    if (this._options.writableObjectMode || this._options.objectMode) {
      debug('%s received object', this.id, chunk);
    } else {
      debug('%s received %d bytes', this.id, chunk.length, chunk.toString());
    }
