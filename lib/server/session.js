  ...
  _bind(socket) {
    ...
    for (var event of boxEvents) {
      (function (eventName) {
        ...
        debug('before socket.on', eventName, fn);
        socket.on(eventName, function () {
            debug('socket.on ', eventName, fn);
    ...
    process.on('attach', function (pid) {
      ...
      if (options.streams && _.isInteger(options.streams)) {
        ...
        for (var i = 0; i < options.streams; i++) {
            streams[fdCounter] = (new RemoteStream(this.socket, this.id + '.' + fdCounter, false, {
              readableObjectMode : options.streamsObjectMode[i],
              writableObjectMode  : options.streamsObjectMode[i]
            }));
            process.stdio[fdCounter].pipe(streams[fdCounter]);
            fdCounter++; // increase
        }
      }
