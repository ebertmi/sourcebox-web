...
var shutdown = _.once(function (signal) {
  ...
  session.cleanup('Sourcebox Server shutting down!')
...
var defaultOptions = {
  ...
  io: {
    serveClient: false,
    origins: '*://trycoding.io:* http://localhost:*'
  }
};
  ...
  _onConnection(socket) {
    ...
    new Promise(function (resolve) {
      ...
      .catch(function (error) {
        ...
        debug('socket %s failed to auth: %s', socket.id, message, error);
        socket.emit('fatal', 'Authentication failed: ' + message);
        socket.disconnect();
      });
  }
